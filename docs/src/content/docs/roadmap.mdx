...
## Current Status: v0.32

TensorDB v0.32 ships a fully functional bitemporal ledger database with:
...
- Structured error codes with "Did you mean?" suggestions
- Audit logging, row-level security, GDPR erasure
- Per-query resource limits, online DDL, plan stability
- VACUUM, VERIFY BACKUP, SUGGEST INDEX, compaction scheduling, WAL management
...
- ~~Immutable audit log~~ (Done — `SHOW AUDIT LOG`, append-only `__audit_log/` prefix)
- ~~GDPR right-to-erasure~~ (Done — `FORGET KEY '<key>' FROM <table>`)
- SOC 2 / HIPAA compliance checklist
- ~~Row-level security policies~~ (Done — `CREATE POLICY ... USING (...)`)
...
| Version | Milestone |
|---|---|
| **v0.32** | Error quality, security & audit, operational maturity |
| **v0.31** | Observability & diagnostics (SHOW commands, /health) |
| **v0.30** | Vector search, horizontal scaling, ecosystem |
