...
impl Wal {
    ...
    pub fn replay(path: impl AsRef<Path>) -> Result<Vec<FactWrite>> {
        ...
        loop {
            ...
            if magic != WAL_MAGIC && magic != WAL_BATCH_MAGIC {
                return Err(TensorError::WalMagicMismatch);
            }
    ...
    /// Replay WAL records up to and including the given epoch.
    /// Records with epoch > max_epoch are excluded. Records with epoch == 0
    /// (legacy/non-transactional) are always included.
    pub fn replay_until_epoch(path: impl AsRef<Path>, max_epoch: u64) -> Result<Vec<FactWrite>> {
        let all = Self::replay(path)?;
        Ok(all
            .into_iter()
            .filter(|w| w.epoch == 0 || w.epoch <= max_epoch)
            .collect())
    }

    /// Replay WAL records for a specific epoch range [from_epoch, to_epoch].
    /// Records with epoch == 0 (legacy) are excluded unless explicitly in range.
    pub fn replay_epoch_range(
        path: impl AsRef<Path>,
        from_epoch: u64,
        to_epoch: u64,
    ) -> Result<Vec<FactWrite>> {
        let all = Self::replay(path)?;
        Ok(all
            .into_iter()
            .filter(|w| w.epoch >= from_epoch && w.epoch <= to_epoch)
            .collect())
    }

    /// Get the highest epoch found in the WAL.
    pub fn max_epoch(path: impl AsRef<Path>) -> Result<u64> {
        let all = Self::replay(path)?;
        Ok(all.iter().map(|w| w.epoch).max().unwrap_or(0))
    }
}
