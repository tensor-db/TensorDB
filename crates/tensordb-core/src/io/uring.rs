...
impl UringWalWriter {
    pub fn open(path: impl AsRef<Path>, queue_depth: u32) -> Result<Self> {
        let file = std::fs::OpenOptions::new()
            ...
            .open(path)?;
        let ring =
            IoUring::new(queue_depth).map_err(|e| TensorError::Io(std::io::Error::from(e)))?;
        Ok(Self { ring, file })
    }
    ...
    pub fn write_and_sync(&mut self, data: &[u8]) -> Result<()> {
        ...
        unsafe {
            self.ring
                .submission()
                .push(&write_e)
                .map_err(|_| TensorError::Io(std::io::Error::other("SQ full")))?;
        }
        ...
        self.ring
            .submit_and_wait(2)
            .map_err(|e| TensorError::Io(std::io::Error::from(e)))?;

        let mut cq = self.ring.completion();
        for _ in 0..2 {
            if let Some(cqe) = cq.next() {
                if cqe.result() < 0 {
                    return Err(TensorError::Io(std::io::Error::from_raw_os_error(
                        -cqe.result(),
                    )));
                }
            }
