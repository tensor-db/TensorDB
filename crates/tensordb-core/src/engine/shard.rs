...
impl ShardReadHandle {
    ...
    /// Bump the shard's commit counter to at least `min_ts`.
    /// Used by `advance_epoch()` to synchronize all shard counters with the
    /// global epoch so that PITR queries can correctly filter cross-shard data.
    pub fn bump_commit_counter(&self, min_ts: u64) {
        self.shared
            .commit_counter
            .fetch_max(min_ts, Ordering::AcqRel);
    }
